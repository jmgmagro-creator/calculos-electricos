...
def normalize_section(s, material="Cu"):
    std = [1.5,2.5,4,6,10,16,25,35,50,
...
def calculate(inp: CircuitInput) -> CircuitOutput:
    ...
    # 3. Corriente ficticia
    out.ib_corrected = round(
        out.ib_amperes / out.total_correction, 3
    )
    ...
    if found is None:
        out.errors.append(
            f"Sin sección para Ib'="
            f"{out.ib_corrected:.1f}A "
            f"[{inp.installation_method}/"
            f"{inp.insulation_type}]"
        )
        return out
    ...
    out.iz_corrected      = round(
        out.iz_amperes * out.total_correction, 2
    )
    ...
    if inp.circuit_type == "SINGLE_PHASE":
        ...
        out.section_neutral_mm2 = (
            out.section_phase_mm2
            if out.section_phase_mm2 <= thr
            else normalize_section(
                out.section_phase_mm2 / 2,
    ...
    out.in_protection_amperes = float(
        next((r for r in ratings if r >= out.ib_amperes),
    ...
    if not out.check_ib_lt_in:
        out.warnings.append(
            f"❌ Ib={out.ib_amperes:.1f}A "
            f"> In={out.in_protection_amperes:.0f}A"
        )
    if not out.check_in_lt_iz:
        out.warnings.append(
            f"❌ In={out.in_protection_amperes:.0f}A "
            f"> Iz={out.iz_corrected:.1f}A"
        )

    # 7. Caída de tensión
    r_km, x_km = get_impedance(
        inp.conductor_material, out.section_phase_mm2
    )
    t_max = 70.0 if inp.insulation_type == "PVC" else 90.0
    alpha = (TEMP_COEFF_CU
             if inp.conductor_material == "Cu"
             else TEMP_COEFF_AL)
    ...
    fac   = SQRT3 if inp.circuit_type == "THREE_PHASE" else 2.0

    if out.section_phase_mm2 < 50:
        du = fac * out.ib_amperes * inp.length_m * r_m
    else:
        du = fac * out.ib_amperes * inp.length_m * (
            r_m * cosp + x_m * sinp
        )
    ...
    lim = VDROP_LIMITS.get(inp.load_type, 3.0)
    out.check_vdrop_ok = (out.voltage_drop_pct <= lim)
    if not out.check_vdrop_ok:
        out.warnings.append(
            f"❌ ΔU={out.voltage_drop_pct:.2f}% > {lim}%"
        )

    # 8. Icc fin de línea
    if inp.circuit_type == "THREE_PHASE":
        z_o = inp.voltage_v / (SQRT3*inp.icc_origin_ka*1000)
    else:
        z_o = inp.voltage_v / (2*inp.icc_origin_ka*1000)

    r0, x0 = get_impedance(
        inp.conductor_material, out.section_phase_mm2
    )
    fz = 1 if inp.circuit_type == "THREE_PHASE" else 2
    zr = z_o + fz*(r0/1000)*inp.length_m
    zx = fz*(x0/1000)*inp.length_m
    zt = math.sqrt(zr**2 + zx**2)

    if inp.circuit_type == "THREE_PHASE":
        out.icc_end_ka = round(
            (inp.voltage_v/(SQRT3*zt))/1000, 4
        )
    else:
        out.icc_end_ka = round(
            (inp.voltage_v/(2*zt))/1000, 4
        )
    ...
    out.overall_ok = (
        out.check_ib_lt_in and out.check_in_lt_iz
        and out.check_vdrop_ok and out.check_icc_breaking
    )
    return out
    ...
    with tab1:
        ...
        with c2:
            p_kw  = st.number_input(
                "Potencia (kW)", 0.01, 5000.0, 7.5, 0.5
            )
            cosphi = st.slider("cosφ", 0.5, 1.0, 0.85, 0.01)
            eta    = st.slider("η", 0.5, 1.0, 0.92, 0.01)
            u_v    = st.selectbox(
                "Tensión (V)", [230.0,400.0,690.0], index=1
            )
            fd     = st.number_input(
                "Factor Demanda", 0.1, 1.0, 1.0, 0.05
            )
        ...
        with c3:
            ...
            lm   = st.number_input(
                "Longitud (m)", 0.1, 5000.0, 25.0, 1.0
            )
            temp = st.number_input(
                "T ambiente (°C)", 10, 75, 40, 5
            )
            ngr  = st.number_input(
                "Nº Agrupados", 1, 30, 1, 1
            )
        ...
        if st.button("⚡ CALCULAR",
            ...
            if out.errors:
                ...
                col = "#4ade80" if out.overall_ok else "#f87171"
                st.markdown(
                    f"<div style='border-left:4px solid {col};"
                    f"padding:0.5rem 1rem;background:#1a2332;"
                    f"border-radius:4px;margin-bottom:1rem'>"
                    f"<b style='color:{col};font-size:1.1rem'>"
                    f"{'✅ OK' if out.overall_ok else '❌ NOT OK'}"
                    f" — {ref} {desc}</b></div>",
                    unsafe_allow_html=True
                )
                ...
                m[2].metric("S fase (mm²)",
                            f"{out.section_phase_mm2:.1f}")
                m[3].metric("Iz real (A)",
                            f"{out.iz_corrected:.2f}")
                m[4].metric("ΔU (%)",
                            f"{out.voltage_drop_pct:.2f}")
                m[5].metric("Icc fin (kA)",
                            f"{out.icc_end_ka:.3f}")

                st.markdown("#### Verificaciones CHECK")
                lim_du = VDROP_LIMITS.get(ltype, 3.0)
                checks = {
                    "Ib ≤ In":       out.check_ib_lt_in,
                    "In ≤ Iz":       out.check_in_lt_iz,
                    f"ΔU ≤ {lim_du}%": out.check_vdrop_ok,
                    "Icc ≤ PdC":     out.check_icc_breaking,
                }
                cols = st.columns(4)
                for i,(name,ok) in enumerate(checks.items()):
                    bg = "#1a4731" if ok else "#4a1a1a"
                    fc = "#4ade80" if ok else "#f87171"
                    cols[i].markdown(
                        ...
                        f"{'✅' if ok else '❌'}</div>"
                        f"<div style='color:{fc};"
                        f"font-size:0.8rem'>{name}</div>"
                        f"</div>",
                        unsafe_allow_html=True
                    )
                ...
                with st.expander("📐 Detalle técnico"):
                    ...
                    with d1:
                        ...
                        st.write(
                            f"Ib' = {out.ib_corrected:.2f} A"
                        )
                    with d2:
                        st.write("**Conductores**")
                        st.write(
                            f"Fase:   {out.section_phase_mm2} mm²"
                        )
                        st.write(
                            f"Neutro: {out.section_neutral_mm2} mm²"
                        )
                        st.write(
                            f"PE:     {out.section_pe_mm2} mm²"
                        )
                        st.write(
                    ...
                    with d3:
                        ...
                        st.write(
                            f"Icc fin = {out.icc_end_ka:.4f} kA"
                        )
